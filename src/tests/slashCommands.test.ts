...
const fixPrompt = provider.prompts[1] ?? '';
...
// Bỏ pipeline: chỉ 1 lần gọi agent trực tiếp, không còn nhiều phase.
assert.ok(!fixPrompt.includes('CURRENT PHASE:'));
await typeText('/pipeline');
input.write('\r');
await wait(80);
assert.equal(provider.calls, 2);
...
input.write('\r');
await wait(80);
assert.ok(stripAnsi(rendered).includes('Chưa có phase history.'));
await typeText('/session');
input.write('\r');
await wait(80);
assert.ok(stripAnsi(rendered).includes('Chưa có runtime session.'));
...
// Không còn session pipeline nên "tiếp tục task" đi qua đường vibe thường, gọi agent 1 lần.
