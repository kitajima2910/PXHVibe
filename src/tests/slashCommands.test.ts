...
await wait(50);
assert.ok(stripAnsi(rendered).includes('PXHVibe'));
...
assert.ok(stripAnsi(rendered).includes('1/50'));
assert.equal(provider.calls, 0);
input.write('\r');
await wait(50);
assert.ok(stripAnsi(rendered).includes('Enter/Esc quay lại'));
input.write('\x1b');
await wait(50);
input.write('\x1b');
await wait(50);
...
input.write('\x1b');
await wait(50);
input.write('\x1b[200~draft one\ndraft two\ndraft three\ndraft four\x1b[201~');
await wait(50);
assert.ok(stripAnsi(rendered).includes('~4 dòng'));
...
await typeText('React là gì?');
input.write('\r');
await waitForCalls(1);
await wait(100);
assert.equal(provider.calls, 1);
assert.ok(provider.lastPrompt.includes('QUICK ANSWER MODE'));
assert.ok(provider.lastPrompt.includes('USER MESSAGE:\n\nReact là gì?'));
assert.ok(!provider.lastPrompt.includes('PIPELINE:'));
...
assert.ok(fixPrompt.includes('- Đọc STATUS.md nếu tồn tại.'));
assert.ok(fixPrompt.includes('- Cập nhật STATUS.md: đã thay đổi gì, file đã sửa, kết quả kiểm tra, vấn đề còn lại.'));
assert.ok(fixPrompt.includes('OUTPUT FORMAT:'));
assert.ok(fixPrompt.includes('AGENT ROLE: PXH Bug Hunter'));
assert.ok(fixPrompt.includes('WORKFLOW: Debug'));
assert.ok(fixPrompt.includes('ACTIVE SKILLS:'));
assert.ok(fixPrompt.includes('Systematic Debugging:'));
assert.ok(fixPrompt.includes('Verification:'));
assert.ok(fixPrompt.includes('TARGET HIỆN TẠI:\nsửa lỗi đăng nhập'));
assert.deepEqual(provider.prompts.slice(1, 6).map((prompt) => /CURRENT PHASE: ([A-Z-]+)/.exec(prompt)?.[1]),
...
assert.ok(stripAnsi(rendered).includes('✓ ANALYZE'));
await typeText('/history');
input.write('\r');
await wait(80);
assert.ok(stripAnsi(rendered).includes('HISTORY · 5 PHASES'));
await typeText('/session');
input.write('\r');
await wait(80);
assert.ok(stripAnsi(rendered).includes('SESSION ·'));
assert.ok(stripAnsi(rendered).includes('Tiến độ'));
...
await wait(200);
assert.equal(provider.calls, 11);
...
assert.ok(provider.lastPrompt.includes('unexpected'));
