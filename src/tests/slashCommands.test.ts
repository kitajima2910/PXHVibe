...
input.write('\r');
await wait(80);
assert.ok(stripAnsi(rendered).includes('simplified mode'));
assert.equal(provider.calls, 0);
for (const command of ['/version', '/about', '/detect', '/doctor', '/mcp', '/history', '/context', '/diff', '/resume', '/retry', '/cancel', '/help']) {
...
assert.ok(commandOutput.includes('History đã bị tắt'));
...
assert.ok(frameHistory.includes('Chế độ \u2192 hmcRules'));
